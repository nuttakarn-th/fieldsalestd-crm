...
import OTAPnL from "./pages/ota/OTAPnL.tsx";
import OTAContentCalendar from "./pages/ota/OTAContentCalendar.tsx";
...
function SupabaseSync() {
  ...
  const loadOTA = useOTAStore((s) => s.loadFromSupabase);
  ...
  // Mount: โหลด users + settings + services + OTA packages (ไม่ต้องการ JWT)
  useEffect(() => {
    loadUsers();
    loadSettings();
    loadServices();
    loadBookings();
    loadOTA();
  }, [loadUsers, loadSettings, loadServices, loadBookings, loadOTA]);

  // JWT เปลี่ยน (ตอน login หรือ restore จาก localStorage) → reload ข้อมูลที่ต้อง auth
  useEffect(() => {
    loadAll();
    loadOTA();
  }, [loadAll, loadOTA, jwtToken]);
      ...
      <BrowserRouter>
        <Routes>
          ...
          <Route path="/ota" element={<RouteGuard><OTALayout /></RouteGuard>}>
            ...
            <Route path="pnl"         element={<OTAPnL />} />
            <Route path="packages"          element={<OTAPackages />} />
            <Route path="platforms"         element={<OTAPlatforms />} />
            <Route path="content-calendar"  element={<OTAContentCalendar />} />
          </Route>
